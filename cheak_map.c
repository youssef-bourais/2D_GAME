...
/*   Updated: 2023/02/18 22:38:06 by ybourais         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (cheak_c(map, p) == 1 && cheak_rect(map, p) == 1 && ft_wall(map, p) == 1 && cheak_other_1(map, p, e1, e2) == 1
        && cheak_other_2(map, p, p1, p2) == 1 && valid_path(visited, p, *p1, *p2) == 1 && cheak_char(map, p) == 1)
        return 1;
    else
        return 0;
}
